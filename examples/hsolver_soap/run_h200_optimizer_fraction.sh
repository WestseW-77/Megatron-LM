#!/usr/bin/env bash
# Profile the PyTorch SOAP optimizer fraction for one manually selected GBS.
...
usage() {
    cat >&2 <<'EOF'
Usage:
  run_h200_optimizer_fraction.sh [--detach] <global_batch_size> [result_root]

The run always uses the frozen H200 formal-training configuration with:
  backend=torch, train_iters=30, profiling window=iterations 20..30.

Arguments:
  global_batch_size  The only experimental variable; run one value at a time.
  result_root        Result parent directory
                     (default: <workspace>/results/soap-8b-formal/h200/optimizer_fraction).

Environment:
  SOAP_H200_MASTER_PORT  torchrun master port (default: 29540).
...
result_root=${2:-$workspace_root/results/soap-8b-formal/h200/optimizer_fraction}
master_port=${SOAP_H200_MASTER_PORT:-29540}
...
analysis_script=$script_directory/analyze_optimizer_fraction.py
...
run_directory=${SOAP_H200_RUN_DIRECTORY:-$result_root/gbs${global_batch_size}/job_${job_tag}_${timestamp}}
...
if (( detach )); then
    ...
    /usr/bin/nohup /usr/bin/setsid \
        ...
        "$script_directory/run_h200_optimizer_fraction.sh" \
        "$global_batch_size" "$result_root" \
...
{
    echo "[Experiment]"
    echo "name: h200_soap_optimizer_fraction_gbs${global_batch_size}"
    echo "purpose: measure the full PyTorch SOAP optimizer fraction of train_step"
    echo "backend: torch"
    ...
    echo "timing method: rank-local CUDA events; one synchronization after iteration $profile_end_iteration"
    echo "rank aggregation: offline maximum across ranks for each iteration"
...
print("H200 PYTORCH SOAP TIMING ENVIRONMENT CHECK: PASS")
PY
